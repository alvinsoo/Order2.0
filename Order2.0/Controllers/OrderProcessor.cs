...
namespace payments_api.Consumer
{
    public class OrderProcessor : BackgroundService
    {
        ...
        private async Task HandleMessageAsync(string content)
        {
            ...
            string[] ordersStatus = tokens[3].Split("|");
            string[] orderId = tokens[2].Split(",");
            string[] cartId = tokens[1].Split(",");


            _logger.LogInformation($"after received {ordersStatus[0]}");
            _logger.LogInformation($"Processing order... {ordersStatus[0]} ");

            await Task.Delay(30000);

            _logger.LogInformation($"will be published with updated OrderStatus {ordersStatus[0]}");
            ...
            using (var channel = connection.CreateModel())
            {
                ...
                string message = "OrderId=" + orderId[0] + ", CartId=" + cartId[0] + ", ordersStatus=Success";
